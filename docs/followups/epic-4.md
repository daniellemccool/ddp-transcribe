...
### Operator interface is the tool itself — wrapper scripts are non-normative (binding Epic 4 premise)

**Found in:** Epic 3 close-out operations session (2026-07-07); operator ruling
recorded as a comment on ADR-0032 the same day.
**Disposition:** Binding design premise for Epic 4 planning, not a work item.
**Trigger to revisit:** Epic 4 brainstorming/planning start — read this before
...
provisioning). They are NOT the operator interface and Epic 4 must not
...
### `triage` runs mute for an hour — no progress output; config echo misleads

**Found in:** First production `triage --dry-run` (2026-07-07, 7,087-row DB).
**Disposition:** Operator-UX gap; fold into Epic 4's operator-facing pass.
**Trigger to revisit:** Epic 4 operator-commands design (natural bundle with
the `status` subcommand and ADR-0017 done-contract work).

Two observed papercuts. (1) A healthy hour-long dry-run emits nothing at the
default `info` level between the startup config line and the final census —
healthy and wedged are indistinguishable without `pgrep`-ing for the curl
heartbeat or restarting under `RUST_LOG=ddp_transcribe=debug` (subprocess
spawn lines). A progress line every N rows (examined/write-offs/probed so
far, ETA at current rate) would close it. (2) The startup config echo logs
