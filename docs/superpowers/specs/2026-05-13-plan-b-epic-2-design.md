...
Phase boundary discipline per AD0019: at Phase 1 close the controller writes `PHASE-1-CLOSE.md` (≤1 page: what landed, current state, Phase 2 entry point) and ends; Phase 2 starts a fresh controller with the spec + close-out doc + Phase 2 task list.
...
| # | Task | Resolves |
|---|------|----------|
| T1 | ADR drafts for AD0021 (schema-version policy), AD0022 (mutator signatures), AD0023 (stale-sweep semantics) via `scripts/adr-fill`. | — |
...
| T6 | `Store::mark_retryable_failure(video_id, kind: &str, message: &str) -> Result<usize>` per AD0006. Same `WHERE status='in_progress' AND claimed_by = ?` predicate. Companion test for stale-claim case. | — |
...
| # | Task | Resolves |
|---|------|----------|
| T12 | ADR drafts for AD0024 (Bug-class supervision + shutdown ORDER), AD0025 (no polling for Plan B), AD0026 (bounded `process::run` capture), AD0027 (orchestrator topology defaults). | — |
| T13 | Bounded `process::run` capture — replace `read_to_end` (`src/process.rs:118–122`) with streaming reader maintaining a `VecDeque<u8>` of size `stderr_capture_bytes`. Symmetric `stdout_capture_bytes`. Rename `ring_buffer_tail` → `tail_excerpt`. Tier 2 test against synthetic stderr flood. **Note:** an "efficiency worktree" run by the operator may land the bounded buffer ahead of Phase 2. If so, T13's scope reduces to rename + symmetric stdout cap + verify the existing Tier 2 test passes after rename. AD0026 records the decision either way. | FOLLOWUPS T6 (bounded capture + `ring_buffer_tail` rename) |
...
| T15 | `async fn fetch_worker(token: CancellationToken, store: SharedStore, fetcher: &dyn VideoFetcher, sender: mpsc::Sender<FetchedItem>, opts: &ProcessOptions) -> Result<()>`. Loops: claim_next → fetcher.acquire → audio::decode_wav → send `(Claim, Vec<f32>, PathBuf)`. Exits on `claim_next == None` (no polling per AD0025). On retryable error: `mark_retryable_failure` and continue. On Bug: return Err. | — |
...
- WhisperEngine teardown can hang (T5 codex review) — Phase 2 T17 / AD0024 shutdown order
...
2. **The shutdown order in AD0024 is load-bearing.** Codex-advisor specifically flagged that the natural sequence (cancel → join → drop senders) deadlocks the engine worker on `blocking_recv`. The correct sequence is `token.cancel()` → drop fetch→transcribe sender → join workers → `engine.shutdown()` LAST. The ADR text must encode this order, not just the components. Without it the spec describes a hang.
...
- **Item #5 (bounded `process::run` capture)**: planned to land BEFORE Phase 2 begins. T13's scope reduces to rename (`ring_buffer_tail` → `tail_excerpt`) + symmetric `stdout_capture_bytes`. AD0026 still records the decision; commit SHA reference notes whether the bounded buffer landed on the worktree or in T13.
...
- **Item #3 full (drop token text from JSON)**: deferred past Epic 2. Would require an AD0010 amendment (the pass-through rule made `id`+`text` load-bearing for filtering specials).
...
- **AD0001** — per-task file split (Epic 2's plan directory: `docs/superpowers/plans/2026-05-13-plan-b-epic-2/`).
- **AD0002** — dead-code suppression strategy; cleanup-on-consumption.
- **AD0003** — deviation honesty in commits (every brief-verbatim divergence disclosed prominently).
- **AD0005** — `test-helpers` Cargo feature for library items needed by integration tests.
- **AD0006** — mutator signatures return `Result<usize>` (Phase 1 T5/T6/T7/T8 all conform).
- **AD0007** — stats counter convention (input-side counters with verb-named fields).
- **AD0008** — pipeline writes artifacts before `mark_succeeded` (Phase 1's classifier dispatch preserves this; Phase 2's pipelined orchestrator preserves it too — fetch workers write nothing, transcribe worker writes artifacts then calls `mark_succeeded`).
- **AD0009–AD0017** — Plan B Epic 1 feature ADRs (Engine API, raw signals, cancellation, GPU verification, audio invariants, non-parallel whisper, parallelism architecture, done-contract).
- **AD0018** — three-tier review protocol with codex-advisor delegated via Sonnet reviewer.
- **AD0019** — subagent report format (≤250-word STATUS/SUMMARY/CHANGED/DEVIATIONS) and phase-boundary controller restart with `PHASE-N-CLOSE.md` handoff.
...
- The architecture is future-proofed for multi-state per AD0016 (single transcribe worker today; Plan C may add `WhisperPool` of N engines without touching the orchestrator's public shape).
...
**Type / API consistency:** Mutator signatures `(video_id, kind, message) -> Result<usize>` consistent across T6/T7. `tokio_util::sync::CancellationToken` referenced consistently in T15/T16/T17/AD0024. `mpsc::Sender<FetchedItem>` (payload `(Claim, Vec<f32>, PathBuf)`) consistent in T15/T16/AD0027.
