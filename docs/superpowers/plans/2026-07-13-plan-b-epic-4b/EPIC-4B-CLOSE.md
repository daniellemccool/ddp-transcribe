...
**The hedge (ADR-0040, `bdc4723`/`2484a23`):** `watch_history.watched_at_raw` (schema v4) preserves the verbatim DDP `Date` string alongside the parsed `i64`, so a wrong guess is a query fix, never a re-ingest. Window filters use day-granularity bounds (`--window-start`/`--window-end`, inclusive UTC calendar dates) precisely because the sub-day ambiguity this verdict leaves open can never flip a row across a day boundary — the residual uncertainty is bounded by design, not by luck.
