...
- **Decision core:** metadata rides the existing yt-dlp invocation (`--no-simulate --print`, bounded 64 KB stdout, zero extra network requests and no second invocation of any kind); the UNPARSED versioned envelope (`{schema, printed}`) is upserted into `video_metadata_raw` before outcome interpretation (failure paths included, zero mutator changes); only `load-metadata` parses, into nullable schema-v5 columns — so parse bugs are re-parse-fixable, never re-fetch (3M-video corpus: fetch is the irreplaceable operation).
- **Guidance must include:** metadata never creates a new failure mode (capture/insert/parse errors log + count, video outcome unchanged — review rejects violations); envelope `schema` field governs loader compatibility (unknown version ⇒ skip, count); the printed field set is deliberately wider than the typed columns; **no caption/subtitle flag may be added to the fetch argv** (see the descope below); engagement counts are point-in-time snapshots keyed by `metadata_fetched_at`.
- **Why must cite:** the 2026-07-28 probe evidence (46/46 title coverage, sticker text structurally absent from yt-dlp), Research API unavailability, and the 2,982,471-unique-video production measurement. Alternatives rejected: `--write-info-json` (per-video file I/O ×3M), separate enrichment pass (doubles network), direct-to-schema parsing (unrecoverable without re-fetch), JSONL sidecar log (torn lines, new operational artifact).
- **The caption/subtitle descope (operator decision 2026-07-28) must be recorded with its evidence trail:** captions were built (listing on the primary argv, content on a guarded secondary invocation) and then removed before merge. Evidence: (a) a subtitle download failure raises `DownloadError` unless `ignoreerrors` is set (`yt_dlp/YoutubeDL.py`, pinned 2026.07.04) — fatal-on-failure, so track downloads could flip a good fetch to a spurious video failure; (b) listing-only capture is not free either — `writesubtitles` runs TikTok's `_get_subtitles` inside PRIMARY extraction (`extractor/common.py` gate → `extractor/tiktok.py`), spending the primary's timeout budget; (c) the initial "0/46 caption tracks ≈ 0%" reading was an artifact of probing WITHOUT the listing flags — a corrected n=11 re-probe measured 4/11 ≈ 36% coverage, so the secondary invocation would really have run on roughly a third of ~3M fetches. Operator adjudication: the yield does not justify the added failure surface and request pressure. Record the corrected ~36% figure — the epic must not close on the retracted ≈0% number. Envelope schema:1 is therefore `{schema, printed}` and `videos` carries 8 metadata columns, not 9.
...
- `state-machine.md`: SCHEMA_VERSION note (v5, what v4→v5 added — table + 8 columns, nothing else), migrate-ladder paragraph (four stages), mutator contract list gains `upsert_metadata_raw` and `apply_metadata_batch` with exact signatures.
- `index.md`: stage summary + ADR table row for the new record.
- `src-vm.md`: operator sequence becomes ingest → process → **load-metadata** → status; one fenced example with `--state-db` BEFORE `load-metadata`; one sentence on replayability.
- **PI-communication implications (state these plainly wherever the epic's coverage is described):** what the pipeline collects is title/description, uploader identity, timestamp, and engagement counts (point-in-time). **Captions/subtitles are excluded, alongside comments** — comments because they are Research-API-only, captions by the 2026-07-28 operator descope above. Spoken content reaches the study only through whisper transcription, not through platform-served caption tracks.
