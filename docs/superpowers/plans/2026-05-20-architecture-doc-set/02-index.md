# Task 2 — Populate `index.md`

**Goal:** Replace every `(TBD)` in `docs/reference/architecture/index.md` with real content, except §3 (donor's journey), which gets *stubbed* paragraphs in this task and real prose in T07. After this task, `index.md` is functionally complete except the walk-through narrative is placeholder.

**ADRs referenced:** none directly authored; the ADR map in §4 references every existing ADR by number.

**Files:**
- Modify: `docs/reference/architecture/index.md`

**Pre-reqs:** T01 complete (skeleton exists).

---

- [ ] **Step 1: Write §1 — What this system is and who it serves**

Replace the §1 `(TBD)` with this exact content:
...
- [ ] **Step 2: Write §2 — Glossary**

Replace the §2 `(TBD)` with this exact content:
...
- [ ] **Step 3: Write §4 — ADR map**

Replace the §4 `(TBD)` with this exact content. The table is sorted by group (Meta-process → Data input → State machine → Transcription → Orchestration → Operations). Deepdive files carry the relevant subset of this table at their end.
...
If a new ADR has landed since this plan was written, add it to the appropriate group. Check with: `ls docs/decisions/*.md | wc -l` should match the row count in this map plus any 00XX-style files not yet integrated.

- [ ] **Step 4: Write §5 — Where to look for X**

Replace the §5 `(TBD)` with this exact content:
...
- [ ] **Step 5: Write §6 — How this doc is maintained**

Replace the §6 `(TBD)` with this exact content:
...
- **In-flight stamp.** `state-machine.md` and `orchestration.md` carry an "as of commit `<sha>`" stamp pointing at the active plan. Removed at epic close.
- **Diagrams.** ASCII only. Currently two: a topology diagram in `orchestration.md`, a state-transition diagram in `state-machine.md`.
```

- [ ] **Step 6: Write §3 — Stub paragraphs for the donor's journey (T07 fills in real prose)**

Replace the §3 `(TBD)` with this exact content. Each paragraph is a *stub* — a placeholder for the real walk-through prose written in T07. The structure (5 stages, one paragraph each, ending with `→ see ...`) is the contract that T07 honors when it replaces them.
...
- [ ] **Step 7: Verify no `(TBD)` markers remain**

```bash
grep -n "(TBD" docs/reference/architecture/index.md
```

Expected: no output (no matches). If any `(TBD)` remains, return to the corresponding step and replace it.

- [ ] **Step 8: Verify line count is in range**

```bash
wc -l docs/reference/architecture/index.md
```

Expected: between 200 and 270 lines. The spec budgets ~230. Slight over/under is fine; over 270 means the index has bloated and should be trimmed (likely culprit: glossary growing duplicate entries, or §6 restating ADRs instead of redirecting).

- [ ] **Step 9: Verify every internal ADR link resolves**

```bash
grep -oP '\(\.\./\.\./decisions/\K[^)]+' docs/reference/architecture/index.md | sort -u | while read f; do
  test -f "docs/decisions/$f" && echo "OK: $f" || echo "MISSING: $f"
done
```

Expected: every line starts with `OK:`. Any `MISSING:` line means the ADR filename is wrong — fix the link.
...
docs(reference): populate architecture/index.md

Sections §1 (system overview), §2 (glossary), §4 (ADR map), §5 (pointer
table), §6 (maintenance) populated. §3 (donor's journey) is stubbed
pending T07 fill-in after the deepdives are written.
