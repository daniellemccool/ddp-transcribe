...
1. **Checkpoint section** beside the existing "Volume sync" material: recommended invocation for uncapped campaign runs —
   ...
   Also document the mid-run rsync reality (observed live 2026-07-29: `file has vanished: …/.work/ytdlp-*/…` during a manual sync): vanished-file warnings for `.work/` transients are expected and benign, BUT rsync exits 24 on them, which (a) breaks `&&` chains after a manual sync and (b) would count every checkpoint cycle as failed once the hook drives this script. The runbook states this and lists the deploy-repo script fixes to hand its owner: `--exclude='.work/'` on the hop-1 rsync, the already-queued `--exclude='*.tmp-*'`, and treating rsync exit 24 as success inside `sync-to-storage.sh`. Until the script fix lands, a non-zero `checkpoints_failed` composed entirely of exit-24 runs is noise, not signal.
