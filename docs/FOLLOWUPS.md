...
- 403-incident 2026-08-06: yt-dlp version drift vs 0033 pattern pins (fleet runs 2026.06.09/2026.07.04 against a 2026.03.17 pin; no observed mismatch) → any yt-dlp upgrade/reinstall, or unexplained `YtDlpOther` growth. **2026-08-19: trigger FIRED — campaign VM on nightly 2026.08.18.122307 (incident-3 remedy); first-order verification passed (IpBlockedMessage byte-identical, validation census clean); watch `YtDlpOther` in full-cap censuses; deploy-role pin now mandatory (stable 2026.07.04 cannot fetch)**
...
- v0.5.0 validation 2026-08-26 (ext. 08-30): **`10240` + exhausted-pool composition redirect-sample** — 10240 is format-mixed (photo posts confirmed, fraction unknown); `NoVideoFormats` exhaustees are live audio-less photo posts (4/4); `IpBlockedMessage` = removed-or-private → before any removal-rate number ships; HEAD-request classifier, stratify by cohort
- v0.5.0 validation 2026-08-26: **photo-post share of successes via envelopes** — live carousels extract (soundtrack audio), so some transcripts are carousel soundtracks → classify from `video_metadata_raw` before transcript-content analysis
