...
- WAF-incident 2026-08-10: **fetch-URL construction belongs in `build_yt_dlp_args`, with its own ADR** — canonical `@<user>/video/<id>` built from `video_id`, retiring the `source_url` DB rewrite; the ADR must also settle impersonation (currently disabled by *removing curl_cffi*, since `impersonate=True` is hardcoded upstream) → **next release; blocks the next ADR-0043 promotion** (delete-and-relaunch reinstates curl_cffi)
...
- WAF-incident 2026-08-10: **two-form `source_url` corpus** — new artifacts carry `@x` URLs, the existing 880,387 carry share URLs (ADR-0010 provenance field); lossless and reconstructible from the PK, but researcher-visible → decide at analysis handoff, or normalise when the pipeline-side fix lands
...
- 403-incident 2026-08-06: mass-instant-failure circuit breaker for the fetch path (60 h × 8 fail/s unobserved; census only writes at run end) → before the next long unattended campaign stretch. **2026-08-11: promoted to highest-value engineering item** — two outages in five days, and the remedy sits on the permitted side of an actively tuned WAF rule
- WAF-incident 2026-08-10: **hourly census check** (zero successes, or nonzero `YtDlpOther`, in the last hour) — the alarm was pre-registered after incident 1 and never added; it would have caught this one in minutes instead of ~7 minutes of luck → before the next unattended stretch
