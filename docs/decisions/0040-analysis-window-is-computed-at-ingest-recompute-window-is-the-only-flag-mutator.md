...
`watch_history.in_window` is computed once at ingest and never re-derived at
query time; after ingest, only the explicit `recompute-window` subcommand may
change it. Window bounds are inclusive UTC calendar dates — `--window-start`
maps to 00:00:00Z inclusive, `--window-end` covers its whole day (the
following day's 00:00:00Z is the exclusive upper bound), an absent side is
unbounded, and both absent means every row is in-window.
...
- Day-granularity windows are deliberate: they absorb the sub-day timezone ambiguity the DDP-timestamp verdict leaves unresolved (UTC-assumed on documentary evidence, empirically open) for all but boundary-adjacent rows — only rows within the ambiguity offset (~1h) of a window edge can be misclassified, and the count of such rows is bounded by the offset.
...
- `recompute-window` requires at least one of `--window-start` / `--window-end` / `--clear` (a bare invocation is a usage error); `--clear` is the deliberate no-filter opt-in, `--dry-run` reports the would-change row count without writing, and the mutator returns the rows actually flipped per the Store row-change contract.
- The verbatim DDP `Date` string persists in `watch_history.watched_at_raw` (schema v4) and is never dropped, normalized, or overwritten once non-NULL — re-ingest backfills NULL raws on existing rows but never touches `in_window`; the raw string is the hedge that makes the timezone verdict non-fatal either way.
