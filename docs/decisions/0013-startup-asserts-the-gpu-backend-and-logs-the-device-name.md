...
- **The hard-fail is `cfg(feature = "cuda")`-gated and fires at construction.** `EXPECTED_BACKEND` is `ExpectedBackend::Gpu` on a `cuda` build (a non-`Gpu` verdict aborts `WhisperEngine::new` with `WhisperInitError::BackendMismatch`) and `Unconstrained` otherwise, where the backend is reported and nothing is asserted — a CPU dev build must not fail. Every build emits the `tracing::info!` backend/device line.
...
- `tests/whisper_engine_init.rs` carries the GPU-gated assertion-path tests alongside the model-load smoke test and shutdown-deadlock guard (`--features cuda -- --ignored`, since a CPU build cannot exercise the mismatch arm); `detect_backend`'s ordered parse is pinned by in-crate `backend_assertion_tests`, which run on any build.
