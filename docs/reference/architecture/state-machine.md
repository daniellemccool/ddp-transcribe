...
- **`record_fetch_failure(video_id, worker_id, label, message, retries, cookies_configured) -> Result<RecordFetchOutcome>`** — Epic 4a ([ADR 0036](../../decisions/0036-in-batch-capped-retry-with-end-of-queue-claim-ordering-fetcher-is-the-liveness-oracle.md)): the in-pipeline retry decision in one IMMEDIATE transaction. Consults the claim's disposition and `attempt_count` against the lifetime cap (`retries + 1`) and returns a typed outcome — `Requeued` (→ `pending`, event `retry_requeued`), `Parked` (a requires-cookie class with no cookies → stays `failed_retryable`, event `cookie_parked`), or `Exhausted` (cap hit → parks in `failed_retryable`). Same `status = 'in_progress' AND claimed_by = ?` stale-claim predicate; a predicate miss returns a no-change outcome. Documents internally how the 0006 row-change contract is honored under the typed return. Replaces the Epic 3 `mark_retryable_failure` in every pipeline caller. (`src/state/mod.rs`)
