...
The `claimed_by` field is set to the calling worker's `worker_id` string. That string is not a UUID: it is `{hostname}-{pid}`, built once per run in `main.rs`'s `Process` arm (`src/main.rs:190`) with the hostname resolved by `hostname_or_default()` — `/proc/sys/kernel/hostname`, falling back to `$HOSTNAME`, then to the literal `"host"`. The shape is deliberate: on the two-instance SRC deployment a claim has to name *which machine* holds it, so that the `was_claimed_by` in a `swept_stale` event's `detail_json` (see `video_events` above, and *Stale-claim sweep* below) identifies the losing instance instead of pointing at an opaque identifier. Before the real-hostname lookup landed, both instances reported `"host"` and attribution leaned on pid ranges alone. All subsequent in-flight mutators (`mark_succeeded`, `record_fetch_failure`, `mark_terminal_failure`) include `AND claimed_by = ?` in their WHERE predicates, so a swept row is never double-updated.
...
- **`upsert_metadata_raw(video_id: &str, worker_id: &str, envelope_json: &str) -> Result<usize>`** — Epic 4c ([ADR 0042](../../decisions/0042-fetch-time-metadata-is-captured-raw-first-parsing-is-a-replayable-post-run-step.md)): inserts the fetch-time metadata envelope into `video_metadata_raw`, stamping `fetched_at = unix_now()`. `INSERT … SELECT … ON CONFLICT(video_id) DO UPDATE` — last-write-wins across retries *among writers that hold the claim*. Claim-guarded since Epic 5b: the source SELECT carries `WHERE video_id = ?  AND claimed_by = ?`, so a worker whose claim was swept out from under it cannot overwrite the newer envelope captured by whoever re-claimed the row (0023 symmetry; previously an accepted tradeoff). The guard rides the statement rather than the call site because the call must stay unconditional on both the success and the failure path — a lost claim surfaces as `Ok(0)`, never an error, so the video's outcome is unchanged either way. Callers treat any `Err` as best-effort (warn + continue). `backfill-metadata` writes unclaimed rows through `insert_metadata_raw_if_missing` instead. (`src/state/mod.rs`)
