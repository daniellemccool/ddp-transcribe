...
- **engine state** — a `whisper_state` value (per-inference scratch space, distinct from the model context); the internal unit of inference concurrency, kept inside the worker behind the `WhisperEngine` wrapper. Per [ADR 0016](../../decisions/0016-architecture-for-parallelism-engine-api-stable-across-single-and-multi-state.md), the engine API is designed to stay stable whether it drives one state or many.
...
- **terminal failure** — a failure that will not succeed on retry (e.g., video deleted upstream). Recorded with `mark_terminal_failure` (pipeline write-off dispatch per [ADR 0033](../../decisions/0033-evidence-derived-failure-taxonomy-with-inline-write-off-of-probe-validated-dead-message-classes.md)) or `triage_mark_terminal` (operator triage verdict per [ADR 0034](../../decisions/0034-operator-triage-subcommand-oembed-oracle-via-curl-subprocess-message-class-fast-path-attempt-capped-requeue.md)), both setting `status = 'failed_terminal'` — see [`state-machine.md`](state-machine.md).
- **triage** — the operator-driven subcommand that empties `failed_retryable`: classifies stored failure messages, probes ambiguous rows via TikTok's oEmbed endpoint, drains dead rows to `failed_terminal`, and requeues recoverable rows to `pending` under an attempt cap (per [ADR 0034](../../decisions/0034-operator-triage-subcommand-oembed-oracle-via-curl-subprocess-message-class-fast-path-attempt-capped-requeue.md)). Defined in `src/triage.rs`; probe oracle in `src/probe.rs`.
...
**Stage 5 — Failure paths.** Every worker error runs through the evidence-derived classifier (`src/failure.rs`, per [ADR 0033](../../decisions/0033-evidence-derived-failure-taxonomy-with-inline-write-off-of-probe-validated-dead-message-classes.md)) into a three-arm verdict: retryable failures route to `Store::mark_retryable_failure` with a taxonomy kind tag (e.g. `NoDataBlocks`, `SensitiveLoginGated`); the two probe-validated write-off message classes route to `Store::mark_terminal_failure` at failure time; Bug-class errors abort the run via the cancellation token. Rows in `failed_retryable` are drained by the operator-run `ddp-transcribe triage` subcommand ([ADR 0034](../../decisions/0034-operator-triage-subcommand-oembed-oracle-via-curl-subprocess-message-class-fast-path-attempt-capped-requeue.md)): message-class write-offs go straight to `failed_terminal`, the rest are probed via TikTok's oEmbed endpoint — dead rows to `failed_terminal`, live rows requeued to `pending` under an attempt cap, with the kind normalized on write-back. Requeued `SensitiveLoginGated` rows can then be fetched with cookies via `process --cookies-file` ([ADR 0035](../../decisions/0035-cookies-scoped-to-sensitivelogingated-retries-only-with-argv-redaction.md)). Stale claims (from a `kill -9` or kernel OOM) are handled by the startup sweep described in Stage 2, which writes no audit row. For the clean-drain shutdown path and the error-triggered cancellation-token path that winds down the `JoinSet`, see [`orchestration.md`](orchestration.md). → see [`orchestration.md`](orchestration.md) and [`state-machine.md`](state-machine.md).
...
| 0034 | Operator triage subcommand: oEmbed oracle, message-class fast path, capped requeue | The two `failed_retryable` exits; triage mutators + audit events. Cross-cuts orchestration (keeps the hot path network-pure). |
...
- **In-flight stamp.** While an epic is actively reshaping the `state-machine.md` or `orchestration.md` subsystem, that file carries an "as of commit `<sha>`" stamp pointing at the active plan; the stamp is removed at epic close. Neither file carries one currently — Plan B Epic 3 (which last reshaped them) has closed.
