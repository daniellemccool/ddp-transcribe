...
The path to the downloaded artifact is **never** parsed from yt-dlp's stdout — stdout is the Epic 4c metadata capture, stored unparsed, and an untagged path line there would corrupt `load-metadata`. The output name is requested via the `-o` template (`{video_dir}/{video_id}.%(ext)s`) and then **discovered by scanning the attempt directory** after yt-dlp exits (`find_single_wav`, `src/fetcher/ytdlp.rs`): exactly one `*.wav` is the success case; zero is `FetchError::MissingOutput`; more than one is `FetchError::AmbiguousOutput`, a distinct failure — the fetcher never picks one, because guessing would transcribe an arbitrary file and stamp it as this video's transcript. Both classify as retryable `ytdlp_other`.

**Per-acquire attempt directories (Epic 5b).** Each `acquire` call creates its own fresh directory `{work_dir}/ytdlp-{video_id}.{pid}-{seq}` (`attempt_dir_path`, `src/fetcher/ytdlp.rs` — the same pid + process-local-sequence convention as `atomic_write`'s tmp names). Nothing is ever reused: a retry, a concurrent fetch worker, and a second process each get their own directory, which is what makes the scan above sound (this invocation is the only writer) and cleanup safe (removing one attempt's directory cannot touch another's in-flight output). `acquire` deliberately does **not** pre-clean earlier `ytdlp-{video_id}.*` directories — one may belong to a live sibling.

Ownership is explicit. A failing `acquire` removes the directory itself (the caller never receives a handle on those paths). On success the directory is handed to the pipeline as `Acquisition::AudioFile { wav, attempt_dir }` and travels as `pipeline::FetchedAudio`, whose lifecycle ends exactly once: removed after `mark_succeeded` commits (never before — [ADR 0008](../../decisions/0008-pipeline-writes-transcript-artifacts-before-mark-succeeded.md) ordering is unchanged, the removal simply replaced the WAV-only removal at the same point), or discarded on a decode/transcribe failure, where the row requeues and its retry re-fetches into a fresh directory. The one deliberate non-consumer is `ProcessOutcome::StaleAfterSuccess`: a concurrent sweep cleared the claim, the row will be re-claimed, and deleting bytes the next claim might want stays forbidden.

Residue from a crash, a `kill`, or a cancelled fetch is collected at startup by `output::artifacts::cleanup_work_dirs(work_dir, stale_claim_threshold)`, called from the `Process` arm next to `cleanup_tmp_files`. It is age-gated on the same threshold and for the same reason: a directory younger than the stale-claim window may belong to a live sibling process mid-fetch. Unreadable mtime means skip and warn (never destroy on uncertainty), and the returned count reports only directories actually removed.
...
`FetchError` also has `WorkDirCreate` (filesystem failure creating the per-acquire attempt dir), `MissingOutput` (yt-dlp exits zero but no WAV is in the attempt dir) and `AmbiguousOutput` (yt-dlp exits zero but the attempt dir holds more than one WAV) variants (`src/errors.rs`).
