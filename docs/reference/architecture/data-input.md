...
The fetcher downloads each claimed video using `yt-dlp` as a subprocess. The orchestrator's fetch workers each invoke the fetcher once per claim; the fetcher returns the local path of the downloaded audio file (or an error), and the file is passed downstream for transcription. Subprocess output is bounded per [ADR 0021](../../madr-archive/0021-bounded-subprocess-output-capture-via-streaming-vecdeque-u8.md) — output streams are drained fully to prevent child processes from blocking, with stderr retaining the trailing 8 KiB and stdout discarded.
