...
The orchestrator turns worker outcomes into state-machine mutations via **three-arm classifier dispatch** (Epic 3, [ADR 0033](../../decisions/0033-evidence-derived-failure-taxonomy-with-inline-write-off-of-probe-validated-dead-message-classes.md)):

- **Fetch-side errors** run through `classify_fetch_phase` (`src/pipeline/mod.rs`), transcribe-side errors through `classify_transcribe_error` (`src/failure.rs`); both produce a `ClassifiedFailure` with three arms. **`Retryable { kind, ctx }`** → `Store::mark_retryable_failure` with the typed kind's `tag()` (e.g. `"NoDataBlocks"`, `"SensitiveLoginGated"`) and a message that leads with the matched classification rule; the worker continues. **`Unavailable { reason, ctx }`** (fetch-side only; the two probe-validated write-off classes per ADR 0033) → `Store::mark_terminal_failure` — its first callers, landed in Epic 3 T07 (`src/pipeline/pipelined.rs`, fetch worker's dispatch; mirrored in `run_serial`) — and the worker continues; the row never retries. **`Bug { ctx }`** → the worker returns `Err`, which the supervisor turns into `token.cancel()` + drain. Classification is message/exit-status-driven and network-pure — no probe on the hot path; liveness probing belongs to the operator `triage` subcommand per [ADR 0034](../../decisions/0034-operator-triage-subcommand-oembed-oracle-via-curl-subprocess-message-class-fast-path-attempt-capped-requeue.md). This reads consistently with [`data-input.md`](data-input.md) §Retry classification and [`state-machine.md`](state-machine.md) §Failure classification.
...
- **Cookie-scoped fetch opts** ([ADR 0035](../../decisions/0035-cookies-scoped-to-sensitivelogingated-retries-only-with-argv-redaction.md)). Each claim's fetch options are computed by `cookie_opts_for` (`src/pipeline/mod.rs`): the operator-supplied `--cookies-file` path rides on the yt-dlp invocation **only** when the claim's `last_retryable_kind` snapshot is `SensitiveLoginGated` — i.e. only on triage-requeued retries of login-gated rows. First attempts never send cookies; the cookie path is redacted from structured subprocess logs and scrubbed from stderr excerpts.
...
| 0034 | Operator triage subcommand | Why the hot path stays network-pure; retry execution lives outside the orchestrator. |
