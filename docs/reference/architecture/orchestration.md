...
Per [ADR 0025](../../decisions/0025-shutdown-order-is-load-bearing-cancel-drain-the-joinset-then-shut-down-the-engine.md), the orchestrator supervises workers with a `tokio::task::JoinSet<Result<()>>` (`src/pipeline/pipelined.rs:520`) plus a shared `tokio_util::sync::CancellationToken` (`:511`; `tokio-util` is in `Cargo.toml:24`). The token is cloned into every worker (`:525`, `:539`).
...
The load-bearing constraint per ADR 0025 is that `engine.shutdown()` runs **last** — after `join_next` has drained the workers and after the caller has dropped its transcriber clone. Reversing this (shutting the engine down before the workers drain) wedges the transcribe worker on a dead engine; the `drop(transcriber)` at `src/commands.rs:264` is the bridge between the worker drain and the engine teardown. Redirect the rationale to ADR 0025.
...
Since Epic 4a, the `Process` arm of `commands::dispatch` (`src/commands.rs` — it lived in `main.rs` until Epic 5b's thin-bin restructure) wraps the orchestrator in a durable batch lifecycle ([ADR 0036](../../decisions/0036-retry-is-in-pipeline-capped-failure-time-requeue-the-re-fetch-is-the-liveness-oracle.md)):

Before the batch opens, the `Process` arm runs a startup tmp sweep — `output::artifacts::cleanup_tmp_files(&cfg.transcripts, cfg.stale_claim_threshold)` (`src/commands.rs:99`) deletes leftover partial-write temp files under the transcripts tree, but only those older than the stale-claim threshold; the age guard is what makes it safe beside a second live instance, whose in-flight temp file is younger than the threshold and is spared. Since Epic 5b a second, identically age-gated sweep runs next to it — `output::artifacts::cleanup_work_dirs(&work_dir, cfg.stale_claim_threshold)` (`src/commands.rs:110`) over `<transcripts>/.work/`, collecting `ytdlp-*` attempt directories left by a crash, a `kill`, or a cancelled fetch. Both are class (a) under [ADR 0047](../../decisions/0047-blocking-io-on-the-worker-hot-path-runs-on-spawn-blocking-inline-only-when-nothing-can-be-starved.md): genuinely unbounded tree walks, which is exactly why they are startup-only and run before any worker spawns.
...
| 0047 | Blocking IO on the worker hot path runs on `spawn_blocking` | The a/b/c classification for sync calls reachable from an async fn. The two class-(c) moves are on this path: `fetch_and_decode`'s `decode_wav` and `transcribe_worker`'s `write_artifacts_durable`. |
